...
static VRTrackingState g_VRTrackingState; // Updated each frame

static inline void QuaternionToEulerDegrees(const XrQuaternionf& q, float& yawDeg, float& pitchDeg, float& rollDeg) {
    // Convert quaternion to Euler angles (yaw-Y, pitch-X, roll-Z) in degrees
    const float ysqr = q.y * q.y;

    // roll (Z-axis rotation)
    float t0 = +2.0f * (q.w * q.z + q.x * q.y);
    float t1 = +1.0f - 2.0f * (q.z * q.z + q.x * q.x);
    ...
    float t4 = +1.0f - 2.0f * (ysqr + q.x * q.x);
...
static inline void UpdatePoseInfo(PoseInfo& out, const XrPosef& pose, XrBool32 valid) {
    out.position = pose.position;
    out.orientation = pose.orientation;
    out.valid = valid;
    QuaternionToEulerDegrees(pose.orientation, out.yawDeg, out.pitchDeg, out.rollDeg);
}
...
struct OpenXrProgram : IOpenXrProgram {
    ...
    VRTrackingState GetTrackingState() const override {
       return g_VRTrackingState;
    }
    ...
    void RenderFrame() override {
        ...
        // }
        if (frameState.shouldRender == XR_TRUE) {
            // Only update tracking, skip GPU rendering
            UpdateTrackingOnly(frameState.predictedDisplayTime);
        }
    ...
    void UpdateTrackingOnly(XrTime predictedDisplayTime) {
        ...
        if (!m_views.empty()) {
            UpdatePoseInfo(g_VRTrackingState.head, m_views[0].pose, XR_TRUE);
        }

        // Update hands
        for (auto hand : {Side::LEFT, Side::RIGHT}) {
            XrSpaceLocation spaceLocation{XR_TYPE_SPACE_LOCATION};
            xrLocateSpace(m_input.handSpace[hand], m_appSpace, predictedDisplayTime, &spaceLocation);
            const bool valid = ((spaceLocation.locationFlags & XR_SPACE_LOCATION_POSITION_VALID_BIT) != 0) &&
                            ((spaceLocation.locationFlags & XR_SPACE_LOCATION_ORIENTATION_VALID_BIT) != 0);
            UpdatePoseInfo(g_VRTrackingState.hand[hand], spaceLocation.pose, valid ? XR_TRUE : XR_FALSE);
        }

        g_VRTrackingState.lastUpdate = predictedDisplayTime;
    }
    ...
    bool RenderLayer(XrTime predictedDisplayTime, std::vector<XrCompositionLayerProjectionView>& projectionLayerViews,
        ...
        // Update tracking snapshot for head using view[0] pose (approximate head pose).
        if (!m_views.empty()) {
            UpdatePoseInfo(g_VRTrackingState.head, m_views[0].pose, XR_TRUE);
        }
        g_VRTrackingState.lastUpdate = predictedDisplayTime;
        ...
        for (auto hand : {Side::LEFT, Side::RIGHT}) {
            ...
            if (XR_UNQUALIFIED_SUCCESS(res)) {
                const bool poseValid = ((spaceLocation.locationFlags & XR_SPACE_LOCATION_POSITION_VALID_BIT) != 0) &&
                                       ((spaceLocation.locationFlags & XR_SPACE_LOCATION_ORIENTATION_VALID_BIT) != 0);
                if (poseValid) {
                    float scale = 0.1f * m_input.handScale[hand];
                    cubes.push_back(Cube{spaceLocation.pose, {scale, scale, scale}});
                }
                // Update tracking snapshot for this hand regardless, with validity flag
                UpdatePoseInfo(g_VRTrackingState.hand[hand], spaceLocation.pose, poseValid ? XR_TRUE : XR_FALSE);
            } else {
                ...
                g_VRTrackingState.hand[hand].valid = XR_FALSE;
            }
        }
